import { NextResponse } from "next/server";
import { createClient } from "@/lib/supabase/server";

export async function POST(request: Request) {
  try {
    const supabase = await createClient();

    // 1. Authenticate user
    const {
      data: { user },
    } = await supabase.auth.getUser();

    if (!user) {
      return NextResponse.json({ error: "Unauthorized." }, { status: 401 });
    }

    // 2. Fetch reviewer member profile
    const { data: reviewer, error: reviewerError } = await supabase
      .from("members")
      .select("id, department, position, status, role")
      .or(`user_id.eq.${user.id},email.eq.${user.email}`)
      .maybeSingle();

    if (reviewerError || !reviewer || reviewer.status !== "active") {
      return NextResponse.json(
        { error: "Forbidden. Active member account required." },
        { status: 403 }
      );
    }

    // 3. Parse and validate body
    const body = await request.json();
    const { task_id, action } = body;

    if (!task_id || !["approve", "reject"].includes(action)) {
      return NextResponse.json(
        { error: "Missing required fields: task_id and action ('approve' or 'reject')." },
        { status: 400 }
      );
    }

    // 4. Fetch the task with assignee info
    const { data: task, error: taskError } = await supabase
      .from("tasks")
      .select("*, members!tasks_assigned_to_fkey(id, xp)")
      .eq("id", task_id)
      .maybeSingle();

    if (taskError || !task) {
      return NextResponse.json({ error: "Task not found." }, { status: 404 });
    }

    if (task.status !== "pending_review") {
      return NextResponse.json(
        { error: "Conflict. Task is not pending review." },
        { status: 409 }
      );
    }

    // 5. Verify review permission
    const posLower = reviewer.position.toLowerCase();
    const isTopTier = posLower.includes("president") || posLower.includes("mentor");
    const isAssigner = task.assigned_by === reviewer.id;

    if (!isAssigner && !isTopTier) {
      return NextResponse.json(
        { error: "Forbidden. You can only review tasks that you assigned, unless you are a President or Mentor." },
        { status: 403 }
      );
    }

    // 6. Handle action
    if (action === "approve") {
      // Begin approval: Update task status
      const { error: taskUpdateError } = await supabase
        .from("tasks")
        .update({
          status: "completed",
          completed_at: new Date().toISOString(),
        })
        .eq("id", task_id);

      if (taskUpdateError) {
        console.error("Error approving task:", taskUpdateError);
        return NextResponse.json({ error: "Failed to update task status." }, { status: 500 });
      }

      // Fetch current XP of the member
      const targetMember = Array.isArray(task.members) ? task.members[0] : task.members;
      const currentXp = targetMember?.xp || 0;
      const newXp = currentXp + task.xp_reward;

      // Update member's XP
      const { error: xpUpdateError } = await supabase
        .from("members")
        .update({ xp: newXp })
        .eq("id", task.assigned_to);

      if (xpUpdateError) {
        console.error("Error updating member XP:", xpUpdateError);
        // Attempt to roll back task status to pending_review
        await supabase
          .from("tasks")
          .update({ status: "pending_review", completed_at: null })
          .eq("id", task_id);

        return NextResponse.json({ error: "Failed to allocate XP reward." }, { status: 500 });
      }

      return NextResponse.json({ success: true, new_xp: newXp });
    } else {
      // Reject: Reset task to assigned, clear proof details
      const { error: rejectError } = await supabase
        .from("tasks")
        .update({
          status: "assigned",
          proof: null,
          submitted_at: null,
        })
        .eq("id", task_id);

      if (rejectError) {
        console.error("Error rejecting task:", rejectError);
        return NextResponse.json({ error: "Failed to reject task." }, { status: 500 });
      }

      return NextResponse.json({ success: true });
    }
  } catch (error) {
    console.error("Error in task review API:", error);
    return NextResponse.json({ error: "Internal Server Error" }, { status: 500 });
  }
}
